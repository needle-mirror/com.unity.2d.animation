...
namespace UnityEngine.U2D.IK
{
    ...
    public static class FABRIK2D
    {
        ...
        {
            foreach (FABRIKChain2D chain in chains)
            {
        ...
        static void Forward(Vector2 targetPosition, float[] lengths, ref Vector2[] positions)
        {
            ...
            for (int i = last - 1; i >= 0; --i)
            {
                ...
                Vector2 position = (1f - l) * positions[i + 1] + l * positions[i];
        ...
        {
            Vector2 localDifference = endPosition - startPosition;
            float angle = Vector2.SignedAngle(right, localDifference);
            Vector2 validatedPosition = endPosition;
            if (angle < min)
            {
                Quaternion minRotation = Quaternion.Euler(0f, 0f, min);
                validatedPosition = startPosition + (Vector2)(minRotation * right * localDifference.magnitude);
            }
            else if (angle > max)
            {
                Quaternion maxRotation = Quaternion.Euler(0f, 0f, max);
