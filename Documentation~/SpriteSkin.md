...
| **Auto Rebind** | Enable this to have the component attempt to find the correct GameObject Transforms to use as bones for the Sprite by using the GameObject Transform set in the **Root Transform** property as the starting point. For more information, refer to [Map bones to transforms automatically](Rebind.md). |
...
| **Root Transform** | Use this property to indicate which GameObject Transform to use as the root when searching for bone Transforms during Auto Rebind. |
...
| **Create Bones** | The button lets you create GameObject Transform(s) to represent the Sprite’s Bone and assign them to the individual Bones entry, and sets the **Root Transform** to the GameObject of the Sprite Skin. The created Bones are placed as children of the GameObject of the Sprite Skin. The button is only enabled if the Root Transform property isn't assigned. |
